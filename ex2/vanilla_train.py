...
disc_point = get_modify(np.array([0.5]), y)
...
train_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(input_f, input_loc_1, input_loc_2, output_1,
                                                                          output_2),
...
for ep in range(epochs):
    ...
    if not (ep + 1) % 100:
        torch.save(model_1.state_dict(), 'ex2/vanilla_model1.pt')
        torch.save(model_2.state_dict(), 'ex2/vanilla_model2.pt')
        print(ep)
...
plt.tight_layout()
plt.show(block=True)
plt.savefig('ex2/vanilla_loss.png')
