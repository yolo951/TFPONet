...
def discontinuous_tfpm(f, eps):
    ...
    def F(x):
        ...
        return interpolate.interp1d(grid, f[k]*val_a)(x)
...
f = -grid**2+5*grid+1
f = f.reshape((1,-1))
u_pred = discontinuous_tfpm(f, 0.001)
# u = np.where(grid<=0.5, 3*sqrt(e)/(3*e-1)*(np.exp(-grid)-np.exp(grid)), 4/(3*e-1)*(1-grid))
# plt.plot(grid, u, 'b-',label='exact solution')
plt.plot(grid, u_pred.flatten(),label='tfpm')
plt.plot(grid, u_exact.flatten(),label='exact')
plt.legend()
plt.savefig('daishan.png')
